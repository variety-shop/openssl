...
static int add_ip_to_cache(SSL_CTX* ctx, struct sockaddr_storage* sstorage)
{
    ...
    if (!SSL_CTX_add_session(ctx, ssl->session)) {
...
{
    RAND_bytes((unsigned char*)&sin6->sin6_addr, sizeof(sin6->sin6_addr));
    RAND_bytes((unsigned char*)&sin6->sin6_port, sizeof(sin6->sin6_port));
...
{
    RAND_bytes((unsigned char*)&sin4->sin_addr, sizeof(sin4->sin_addr));
    RAND_bytes((unsigned char*)&sin4->sin_port, sizeof(sin4->sin_port));
...
int main(int argc, char *argv[])
{
    ...
    /* Try to search for an address that should be found */
    ...
    SSL_CTX_free(ctx);
#ifndef OPENSSL_NO_CRYPTO_MDEBUG
    if (CRYPTO_mem_leaks(bio_stdout) <= 0)
        ret = 1;
#endif
    BIO_free(bio_stdout);
    return ret;
