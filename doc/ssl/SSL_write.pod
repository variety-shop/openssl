...
SSL_write, SSL_writev - write bytes to a TLS/SSL connection
...
SSL_writev() writes B<count> number of data buffers from the list of B<buckets>
into the specified B<ssl> connection.
...
Before calling SSL_writev(), each element in the B<buckets> array must be filled
with buffers and lengths of those buffers to write data from. The maximum number
