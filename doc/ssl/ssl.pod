...
=item int B<SSL_CTX_set_ciphers_ex>(SSL_CTX *ctx, char *str, unsigned long flags);
...
=item int B<SSL_CTX_use_no_PrivateKey>(SSL_CTX *ctx);
...
=item int B<SSL_use_no_PrivateKey>(SSL *ssl);
...
L<SSL_CTX_set_ciphers_ex(3)|SSL_CTX_set_ciphers_ex(3)>,
