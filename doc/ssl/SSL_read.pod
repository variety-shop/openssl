...
SSL_read, SSL_readv - read bytes from a TLS/SSL connection
...
SSL_readv() tries to read as many bytes as specified by B<count> of B<buckets>
from the specified B<ssl> into the buffers specified by B<buckets>.
...
The B<SSL_BUCKET> struture is defined as a typedef of B<struct iovec>,
if available, or a structure that mirrors it:

  typedef struct iovec SSL_BUCKET;

Or:

  typedef ssl_bucket_st SSL_BUCKET;
  struct ssl_bucket_st {
    void *iov_base;
    size_t iov_len;
  }

The B<buckets> argument is an B<count>-element array of B<SSL_BUCKET> strutures.
Before calling ssl_readv(), each element in the B<buckets> array must be filled
with buffers and lengths of those buffers to read data into. The maximum number
of buckets is defined as SSL_BUCKET_MAX.
