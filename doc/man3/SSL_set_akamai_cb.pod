...
SSL_set_akamai_cb, SSL_get_akamai_cb, SSL_CTX_set_akamai_cb,
SSL_CTX_get_akamai_cb - handle event callback function
...
=item B<SSL_AKAMAI_CB_SERVER_SIGN_CERT_VRFY>

The event indicates that the certificate verification message on the server side
needs to be signed.
B<SSL_AKAMAI_CB_DATA.retval> should be set to the length of the signature on success.
...
SSL_get_akamai_cb() returns the current callback or NULL if not set.
