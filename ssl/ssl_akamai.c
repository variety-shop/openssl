                                       ...
                                       int idx, long argl, void* argp)
{
    ...
    const size_t keylen = (sizeof(ctx->ext.tick_hmac_key) +
                                      ...
                                      int idx, long argl, void* argp)
{
    ...
    if (new->tlsext_tick_hmac_key != NULL) {
        SSL_CTX *ctx; /* for sizeof */
        const size_t keylen = sizeof(ctx->ext.tick_hmac_key) + sizeof(ctx->ext.tick_aes_key);
        if ((new->tlsext_tick_hmac_key = OPENSSL_secure_malloc(keylen)) == NULL)
            ...
            new->tlsext_tick_aes_key = new->tlsext_tick_hmac_key + sizeof(ctx->ext.tick_hmac_key);
        }
    }
                                ...
                                STACK_OF(X509) *extra, int override)
{
    ...
    if (ssl_cert_lookup_by_pkey(privatekey, &i) == NULL)
        goto out;
